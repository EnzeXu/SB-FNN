...
class FourierModel(FourierModelTemplate):
    ...
    def real_loss(self, y, y_truth):
        ...
        y, y_truth = y[:, 10:15], y_truth[:, 10:15]
    ...
    def ode_gradient(self, x, y):
        ...
        for ii in range(k.n):
            s_t_collection.append(
                torch.gradient(s[:, ii], spacing=(x,))[0].reshape([-1, 1]))
            i_t_collection.append(
                torch.gradient(i[:, ii], spacing=(x,))[0].reshape([-1, 1]))
            r_t_collection.append(
                torch.gradient(r[:, ii], spacing=(x,))[0].reshape([-1, 1]))
        ...
        for ii in range(k.n):
            tmp_s_t_target = torch.zeros([len(x), 1]).to(self.config.device)
            tmp_i_t_target = torch.zeros([len(x), 1]).to(self.config.device)
            tmp_r_t_target = torch.zeros([len(x), 1]).to(self.config.device)
            for jj in range(k.n):
